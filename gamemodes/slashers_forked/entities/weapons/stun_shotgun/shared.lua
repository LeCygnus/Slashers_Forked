...
SWEP.Instructions = "Stuns killer between 10 to 18 seconds. Stun won't stack on an already stunned killer."
...
SWEP.Primary.Knockback = 500 -- Recoil --
SWEP.Primary.KickUp = 100
...
if SERVER then
	local function EntityTakeDamage(target, dmg)
		if !target:IsPlayer() || !dmg:GetAttacker() || !dmg:GetAttacker().GetActiveWeapon || !dmg:GetAttacker():GetActiveWeapon() ||
			dmg:GetAttacker():GetActiveWeapon():GetClass() != "stun_shotgun" then return end
		if target:Team() == TEAM_SURVIVORS then return true end
		if target:Team() == TEAM_KILLER && !target.stun then
			timer.Create("stunshotgun_" .. target:UniqueID(), math.random(10, 18), 1, function()
	...
	hook.Add("EntityTakeDamage", "stunshotgun_EntityTakeDamage", EntityTakeDamage)
end